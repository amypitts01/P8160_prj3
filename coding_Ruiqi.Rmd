...
log_posterior_mu = function(theta, sigma_p){
  ...
  sigma = theta[3411]
  mu = theta[3406:3410]
  return(-(1/2)*sum(diag(sigma_p))-(1/2)*sum(mcmapply(function(x){t(x-mu)%*%sigma_p%*%(x-mu)}, x = beta, mc.cores = numCores)))
}
...
log_posterior_sigmap = function(theta){
  ...
  if(min(eigen(sigma_p)$values) <= 0){
   ...
   return((d+1+m/2)*log(det(sigma_p))-(1/2)*sum(diag(sigma_p))-(1/2)*sum(mcmapply(function(x){t(x-mu)%*%sigma_p%*%(x-mu)}, x = beta, mc.cores = numCores)))
...
search_a=function(a, theta_0, nrep=10000, n_theta){
  theta = theta_0
  theta_chain = matrix(0, nrow = nrep, ncol = n_theta)
  for(i in 1:nrep){
    sigma_p = matrix(0,5,5)
...
```{r}
a = c(rep(c(0.1,0.01,0.3,0.3,0.2), each =681),0.01,0.01,0.01,0.01,0.01,1,2,2,2.5,2.5,2,3,2,2,2,3,1.5,2,2,2,2)
...
theta_chain = blockwiseMH(a, theta_0, nrep=1, n_theta)
numunique(theta_chain[,1])

x = search_a(a, theta_0, nrep=100, n_theta)
apply(x[,c(1, 1+681, 1+681*2, 1+681*3, 1+681*4,3406:3410,3411,3412:3426)], MARGIN = 2, numunique)/100
```
