...
where $M$ is total number of observation for all hurricanes.
...
colnames(output2) = c("$\\boldsymbol \\beta_0$", "$\\boldsymbol \\beta_1$", "$\\boldsymbol \\beta_2$", "$\\boldsymbol \\beta_3$", "$\\boldsymbol \\beta_4$")
rownames(output2) = c("$\\boldsymbol \\beta_0$", "$\\boldsymbol \\beta_1$", "$\\boldsymbol \\beta_2$", "$\\boldsymbol \\beta_3$", "$\\boldsymbol \\beta_4$")
output2 %>% knitr::kable(escape = FALSE, booktabs = T, caption = "Bayesian Estimate for $\\Sigma^{-1}$")
```
...
dt_rsquare = dt_res %>% 
  ...
  dplyr::select(id, r_square, adj_r_square) %>% 
  distinct()

d_high = dt_rsquare %>% 
  filter(adj_r_square >=0.7 & adj_r_square <= 1) %>% 
  nrow()
d_moderate = dt_rsquare %>% 
  filter(adj_r_square >= 0.3 & adj_r_square< 0.7) %>% 
  nrow()

d_low = dt_rsquare %>% 
  filter(adj_r_square < 0.3 & adj_r_square> 0) %>% 
  nrow()

d_bad = dt_rsquare %>% 
  filter(adj_r_square <= 0)%>% 
  nrow()

label_1 = c("$R_{adj}^2$", "Count of Hurricanes","Percentage($\\%$)")
tibble(adj_r = c("0.7-1", "0.3-0.7", "0-0.3", "< 0"),
       count = c(d_high, d_moderate, d_low, d_bad),
       prop = c(round(d_high/681,3)*100, round(d_moderate/681,3)*100, 
                round(d_low/681,3)*100, round(d_bad/681,3)*100)) %>% 
  kable(escape = F, col.names = label_1, caption = "$R_{adj}^2$ for each hurricane")
...
dt_res %>% filter(id %in% c("INVEST.2010","BECKY.1958", 
                            "DORIS.1975","ANDREW.1986",
                            "FLORENCE.2000", "SANDY.2012")) %>%
  mutate(time = 1:n()) %>% 
