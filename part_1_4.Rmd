...
colnames(output1) = c("$\\boldsymbol \\mu_0$", "$\\boldsymbol \\mu_1$", "$\\boldsymbol \\mu_2$", "$\\boldsymbol \\mu_3$", "$\\boldsymbol \\mu_4$", "$\\boldsymbol \\sigma^2$")
rownames(output1) = "Estimates"
output1 %>% knitr::kable(caption = "Bayesian Estimates for $\\boldsymbol \\mu$ and $\\boldsymbol \\sigma^2$", escape = FALSE) %>% 
  kable_styling(latex_options = "hold_position")
```

```{r}
sigmap = bs_estimates[3412:3426]
row1 = c(round(sigmap[1], 2), round(sigmap[2], 2), round(sigmap[4], 2), round(sigmap[7], 2), round(sigmap[11], 2))
row2 = c(round(sigmap[2], 2), round(sigmap[3], 2), round(sigmap[5], 2), round(sigmap[8], 2), round(sigmap[12], 2)) 
row3 = c(round(sigmap[4], 2), round(sigmap[5], 2), round(sigmap[6], 2), round(sigmap[9], 2), round(sigmap[13], 2))
row4 = c(round(sigmap[7], 2), round(sigmap[8], 2), round(sigmap[9], 2), round(sigmap[10], 2), round(sigmap[14], 2))
...
colnames(output2) = c("$\\boldsymbol \\beta_0$", "$\\boldsymbol \\beta_1$", 
                      "$\\boldsymbol \\beta_2$", "$\\boldsymbol \\beta_3$", 
                      "$\\boldsymbol \\beta_4$")
rownames(output2) = c("$\\boldsymbol \\beta_0$", "$\\boldsymbol \\beta_1$", 
                      "$\\boldsymbol \\beta_2$", "$\\boldsymbol \\beta_3$", 
                      "$\\boldsymbol \\beta_4$")
output2 %>% knitr::kable(escape = FALSE, booktabs = T, 
                         caption = "Bayesian Estimate for $\\Sigma^{-1}$") %>% 
  kable_styling(latex_options = "hold_position")
```
...
dt_res = dt %>% 
  ...
  mutate(r_square = (1-sum((wind_kt_pred-wind_kt)^2)/(sum((wind_kt-mean(wind_kt))^2))),
         ...
         p_value = 1-pchisq(sum((wind_kt_pred-wind_kt)^2)/sigma, n_obs-5))
...
The overall adjusted $R^2$ of the estimated Bayesian model is `r adj_r_square` and P-value for goodness-of-fit test of all observations is `r p_value`. 


```{r}
dt_rsquare = dt_res %>% 
  dplyr::select(id, r_square, adj_r_square, p_value) %>% 
  distinct()
...
We also perform the goodness-of-fit test for individual estimated model. `r n_lowp` of the estimated models have p-value less than 0.05, implying that those models do not fit well.   

```{r}
## visualize some hurricane prediction

dt_res %>% filter(id %in% c("DOG.1950","JANET.1955","KING.1950",
                            "BARBARA.1954","GINGER.1967", "ARLENE.1971",
                            "ALFA.1973","JERRY.2007","JENNY.1969")) %>% 
  ungroup() %>% 
