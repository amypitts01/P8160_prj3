...
- We used the lag difference ($t-6$ to $t-12$) for latitude, longitude and wind speed to build $\Delta_{i,1}(t),\Delta_{i,2}(t), \Delta_{i,3}(t)$ and lag of wind speed as $Y_{i}(t-6)$
...
- Post-selection Inference
  - bootstrap smoothing as proposed by Efron (2014)
    - fit full model via Poisson-family GLM to obtain $\hat{\boldsymbol\mu}$
    - for single bootstrap $b$, draw $\mathbf{y}_b^* \sim \text{Poisson}(\hat{\boldsymbol\mu})$
    - execute lasso with $\mathbf{y}_b^*$ as output to obtain distribution of $\hat{\gamma}_b^*$
...
-  Based on the estimated $\mu$, on average, the increase of previous wind speed and larger increase of speed in previous time interval are corresponding with increase of current wind speed, while the wind will be slower when the hurricane moves further to north and east. 
- Hurricanes with more observations enjoy better MCMC estimates
- With such a high-dimensional parameter space:
  - burn-in takes longer than expected
